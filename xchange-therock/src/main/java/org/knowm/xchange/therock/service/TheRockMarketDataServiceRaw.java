...
public class TheRockMarketDataServiceRaw extends TheRockBaseService {
  ...
  public TheRockFunds getFunds() throws IOException {
    TheRockFunds data = this.theRock.getFunds();
    return data;
  }

  public org.knowm.xchange.therock.dto.marketdata.TheRockTicker getTheRockTicker(TheRock.Pair currencyPair) throws TheRockException, IOException {
    return this.theRock.getTicker(currencyPair);
  }

  public TheRockOrderBook getTheRockOrderBook(TheRock.Pair currencyPair) throws TheRockException, IOException {
    return this.theRock.getOrderbook(currencyPair);
  }
  ...
  public TheRockTrades getTheRockTrades(TheRock.Pair currencyPair, Object[] args) throws IOException {
    ...
    if (args.length == 1) {
      ...
      if ((arg instanceof Number)) {
        after = new Date(((Number) arg).longValue() * 1000L);
      } else if ((arg instanceof Date)) {
        after = (Date) arg;
      }
    }

    return this.theRock.getTrades(currencyPair, after);
  }
}