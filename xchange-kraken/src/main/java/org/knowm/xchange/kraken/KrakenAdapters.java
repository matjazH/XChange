...
public class KrakenAdapters {
  ...
  public static Currency adaptCurrency(String krakenCurrencyCode) {
    String currencyCode = (krakenCurrencyCode.length() == 4)
        && ((krakenCurrencyCode.startsWith("X")) || (krakenCurrencyCode.startsWith("Z"))) ? krakenCurrencyCode.substring(1) : krakenCurrencyCode;
  ...
  public static LimitOrder adaptLimitOrder(KrakenOrder krakenOrder, String id) {
    ...
    if (orderDescription.getAssetPair().length() > 6) {
      baseCurrency = orderDescription.getAssetPair().substring(0, 4);
      tradeCurrency = orderDescription.getAssetPair().substring(4);
    } else {
      baseCurrency = orderDescription.getAssetPair().substring(0, 3);
      tradeCurrency = orderDescription.getAssetPair().substring(3);
    }
    Currency tradableIdentifier = adaptCurrency(baseCurrency);
    Currency transactionCurrency = adaptCurrency(tradeCurrency);
