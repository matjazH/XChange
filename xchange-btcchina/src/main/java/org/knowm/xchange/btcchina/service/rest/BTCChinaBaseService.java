...
public class BTCChinaBaseService extends BaseExchangeService implements BaseService {
  ...
  public BTCChinaBaseService(Exchange exchange) {
    ...
    this.btcChina = RestProxyFactory.createProxy(BTCChina.class, exchange.getExchangeSpecification().getSslUri(), createClientConfig(exchange.getExchangeSpecification()));
    signatureCreator = BTCChinaDigest.createInstance(exchange.getExchangeSpecification().getApiKey(),
  ...
  public static <T extends BTCChinaResponse> T checkResult(T returnObject) {
    ...
    if (returnObject.getError() != null) throw new BTCChinaExchangeException(returnObject.getError());
