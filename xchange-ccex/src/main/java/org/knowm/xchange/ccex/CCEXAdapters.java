...
public class CCEXAdapters {
  ...
  public static Trades adaptTrades(CCEXTrades cCEXTrades, CurrencyPair currencyPair) {
    ...
    for (CCEXTrade cCEXTrade:cCEXTradestmp) trades.add(adaptCCEXPublicTrade(cCEXTrade, currencyPair));
  ...
  public static Wallet adaptWallet(List<CCEXBalance> balances) {
    ...
    for (CCEXBalance balance : balances) {
      wallets.add(new Balance(Currency.getInstance(balance.getCurrency().toUpperCase()), balance.getBalance(),
          balance.getAvailable(),balance.getBalance().subtract(balance.getAvailable()).subtract(balance.getPending()),
  ...
  public static List<LimitOrder> adaptOpenOrders(List<CCEXOpenorder> cCexOpenOrders) {
    ...
    for (CCEXOpenorder order: cCexOpenOrders) {openOrders.add(adaptOpenOrder(order));}
  ...
  public static List< UserTrade > adaptUserTrades(List<CCEXOrderhistory > cCEXOrderhistory) {
    ...
    for (CCEXOrderhistory cCEXTrade: cCEXOrderhistory) trades.add(adaptUserTrade(cCEXTrade));
    return trades;
  }
  ...
  public static UserTrade adaptUserTrade(CCEXOrderhistory trade) {
    ...
    if(price == null) price = trade.getLimit();
